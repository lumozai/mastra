...
### For HTTP/Protobuf Providers (SigNoz, New Relic, Laminar, MLflow, Latitude, Lumoz)
...
Lumoz uses the `custom` provider configuration shown below. Set `LUMOZ_API_KEY` in `client_id:client_secret` format, optionally set `LUMOZ_ENDPOINT`, and pass those values into the custom config.
...
[Lumoz](https://lumoz.ai) is an agentic production ops platform that discovers unknowns in all agent traffic, from dev to production — new cohorts, unbuilt use cases, silent model/API changes, agent capability & coordination gaps — and ships the fix + the eval. Use the `custom` provider with HTTP/Protobuf, send traces to the Lumoz OTLP endpoint, and authenticate with `LUMOZ_API_KEY` in `client_id:client_secret` format.
...
const lumozApiKey = process.env.LUMOZ_API_KEY

if (!lumozApiKey) {
  throw new Error('LUMOZ_API_KEY is required')
}
      ...
      otel: {
        ...
        exporters: [
          new OtelExporter({
            provider: {
              custom: {
                endpoint: process.env.LUMOZ_ENDPOINT || 'https://api.lumoz.ai/proxy/v1/traces',
                protocol: 'http/protobuf',
                headers: {
                  accept: 'application/x-protobuf',
                  authorization: `Basic ${Buffer.from(lumozApiKey, 'utf-8').toString('base64')}`,
                  ...(process.env.LUMOZ_SUBTENANT_ID
                    ? { 'x-lumoz-subtenant-id': process.env.LUMOZ_SUBTENANT_ID }
                    : {}),
                },
              },
            },
...
:::tip Lumoz-native exporter

For Lumoz-specific setup, use the dedicated [`@mastra/lumoz`](/docs/observability/integrations/exporters/lumoz) exporter instead. It handles Basic auth header construction, the default Lumoz endpoint, and optional subtenant headers for you.
...
| Lumoz | HTTP/Protobuf | `@opentelemetry/exporter-trace-otlp-proto` |
